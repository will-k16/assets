/*BallObbyMobileCameraController script that handles mobile camera by Emre Şahin - emolingo games */
var BallObbyMobileCameraController = pc.createScript('ballObbyMobileCameraController');

// Script attributes to control the sensitivity of the camera look with touch
BallObbyMobileCameraController.attributes.add("touchLookSensitivity", { type: "number", default: 0, title: "Touch Look Sensitivity" });

// 'Snappiness' factor (how fast does the camera reach the target rotation and distance)
BallObbyMobileCameraController.attributes.add("snappinessFactor", { type: "number", default: 0.1, title: "Snappiness Factor", description: "Lower is faster" });


BallObbyMobileCameraController.prototype.initialize = function () {
...
BallObbyMobileCameraController.prototype.reset = function () {
    if (this.app.ballObbyTargetStage) {
        const stageScript = this.app.ballObbyTargetStage.script.ballObbyStage;
        if (stageScript.dirK) {
            this.eulers.x = 90;
        } else if (stageScript.dirM) {
            this.eulers.x = 0;
        } else if (stageScript.dirRK) {
            this.eulers.x = -90;
        } else if (stageScript.dirRM) {
            this.eulers.x = 180;
        } else {
            this.eulers.x = -90;
        }
...
BallObbyMobileCameraController.prototype.setSensitivity = function () {
    const data = parseInt(BallObby_Utils.getItem("mouseSensitivity"));
...
BallObbyMobileCameraController.prototype.addEventCallbacks = function () {
...
BallObbyMobileCameraController.prototype.removeEventCallbacks = function () {
    if (this.app.isMobile) {
...
BallObbyMobileCameraController.prototype.moveCamera = function (edx, edy, sensitivity) {
...
BallObbyMobileCameraController.prototype.postUpdate = function (dt) {
...
BallObbyMobileCameraController.prototype.onTouchStart = function (event) {
...
BallObbyMobileCameraController.prototype.onTouchMove = function (event) {
...
BallObbyMobileCameraController.prototype.getYaw = function () {
    var forward = this.entity.forward.clone();
    return Math.atan2(-forward.x, -forward.z);
};


BallObbyMobileCameraController.prototype.getPitch = function (quaternion, yaw) {
...
BallObbyMobileCameraController.prototype.getWorldPoint = function () {
